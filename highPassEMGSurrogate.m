...
hpFilt = designfilt('highpassfir','StopbandFrequency',450, ...
         'PassbandFrequency',500,'PassbandRipple',0.5, 'SampleRate', 2000, ...
         'StopbandAttenuation',65,'DesignMethod','kaiserwin');
% fvtool(hpFilt) % to check filter pass band and ripple

x1 = V1_HPC_L.values(1:100000);
x2 = V1_HPC_R.values(1:100000);
x3 = V1_EMG.values(1:100000);
x4 = V1_ECog.values(1:100000)

y_HPC_L = filtfilt(hpFilt,x1);
y_HPC_R = filtfilt(hpFilt,x2);
y_EMG = filtfilt(hpFilt,x3);
y_ECog = filtfilt(hpFilt,x4)

windowedCorr = movcorr(y_HPC_R,y_HPC_L,2000);
windowedCorrECog = movcorr(y_EMG,y_ECog,2000);
...
plot(y_EMG)
yyaxis right
plot (windowedCorrECog)
title('high frequency HP LFP correlation against EMG')
legend('EMG','correlation L & R HPC > 500Hz')
